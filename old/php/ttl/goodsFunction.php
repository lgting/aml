...
function GetGoodsList(){
	...
	$result = $DB->query("SELECT id,gname,message,price,type,icon,buyType,showType,type from xl_goods where isshow = 1 and type=0 limit ?, ?",array($startPageNum,$pageNum));			
...
function GetHomeGoodsList(){
	$DB = new Db(DBHOST, DATABASE, DBUSER, DBPASS);
	$result = $DB->query("SELECT id,gname,message,price,type,icon,showType,buyType,buynum,topicon from xl_goods where isshow = 1 and showType<3 and type<2");	
