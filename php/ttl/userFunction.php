...
function GetUserInfoClient(){
	...
	$resultArr['store'] = [];
	$resultArr['belongstore'] = [];
	$curuserRole = $result[0]['role'];
	if($curuserRole==Boss){
		$storeShop = $DB->query("select * from xl_store where bossid = ?",[$userid]);
		$resultArr['store'] = $storeShop;
	}
	else if($curuserRole==User || $curuserRole==Beauties){
		$belogetores = $DB->query("select * from xl_clientslist where clientid = ?",[$userid]);
//		echo json_encode($belogetores);
		if(count($belogetores)>0){
			$resultArr['belongstore'] = $DB->query("select * from xl_store where bossid = ?",[$belogetores[0]['belongto']]);
		}
	}
