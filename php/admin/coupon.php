...
function couponlist() {
	...
	$userresult = $DB -> query("select * from xl_coupon order by id desc limit ?,?", array($startPage, $endPage));
...
function addcoupon() {
	...
	if(!isset($_POST['redpacket'])){
		ErrorSend('not redpacket');
		exit();
	}
	...
	$userresult = $DB -> query("insert into xl_coupon(cname,message,iconurl,type,starttime,endtime,addtime,redpacket)values(?,?,?,?,?,?,?,?)", 
	array($cname, $msg,$iconurl,$ctype,$starttime,$endtime,time(),$redpacket));
	$DB -> Closeconnection();
	ErrorSend('ok');
}
...
function editcoupon(){
	...
	$userresult = $DB -> query("update xl_coupon set cname=?,message=?,iconurl=?,type=?,starttime=?,endtime=?,redpacket=? where id=?", 
	array($cname, $msg,$iconurl,$ctype,$starttime,$endtime,$redpacket,$cid));
	$DB -> Closeconnection();
	ErrorSend('ok');
}
...
function bindcoupon(){
	...
	for($i=0;$i<$bindArrLen;$i++){
		$bindGoodArr = explode('-',$bindArr[$i]);
		$bindid = $bindGoodArr[0];
		$bindNum  = $bindGoodArr[1];
		
		$bindNum = $bindNum<=0?1:$bindNum;
		
		$result = $DB -> query("select * from xl_coupontogoods where couponid =? and goodsid=?",array($bindid,$goodsid));
		if(count($result)>0){
			$DB -> query("update xl_coupontogoods set nums = ? where id=?",array($bindNum,$result[0]['id']));
			continue;
		}		
		$DB -> query("insert into xl_coupontogoods (goodsid,couponid,nums)values(?,?,?)",array($goodsid,$bindid,$bindNum));
	}
